...
class OVSQuantumAgent(object):
    ...
    def update_vm_vlan(self, net_uuid, new_seg_id):
        ...
        if lvm.network_type == constants.TYPE_VLAN:
            ...
            return
        elif lvm.network_type == constants.TYPE_GRE:
            # not support
            pass
        elif lvm.network_type == constants.TYPE_LOCAL:
            # not support
    ...
    def vm_eth_binding(self, old_bindings):
        ...
        eths_mac = utils.get_eth_mac_list() # {'eth1': '00:19:e0:77:7a:5c', 'eth0': '6c:62:6d:d2:ea:f2'}
        new_bindings = {}
        port_names = []
        if self.enable_tunneling:
            port_names = self.tun_br.get_port_name_list() # ['eth1']
        else:
...
def main():
    ...
    tun_br = cfg.CONF.RGOS.tunnel_bridge
    local_ip = cfg.CONF.RGOS.local_ip
    enable_tunneling = cfg.CONF.RGOS.enable_tunneling
