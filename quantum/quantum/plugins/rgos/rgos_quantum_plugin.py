...
class RgosQuantumPlugin(db_base_plugin_v2.QuantumDbPluginV2,
    ...
    def __init__(self, configfile=None):
        ...
        if self.tenant_network_type not in [constants.TYPE_LOCAL,
            ...
            sys.exit(1)
        self.enable_tunneling = cfg.CONF.RGOS.enable_tunneling
    ...
    def _init_rgos_remote(self):
        ...
        for x in h_allconfig:
            ...
            LOG.debug("Liupu DEBUG: Init remote ssh_host ip: %s" % ssh_host)
            LOG.debug("Liupu DEBUG: Init retry_maxtimes: %s" % retry_maxtimes)
            LOG.debug("Liupu DEBUG: Init reconnect_interval: %s" % reconnect_interval)
            LOG.debug("Liupu DEBUG: Init ssh_port: %s" % ssh_port)
            LOG.debug("Liupu DEBUG: Init remote_switch_user: %s" % username)
            LOG.debug("Liupu DEBUG: Init remote_switch_pass: %s" % passwd)

            t = sshclient.ssh_connect(username, passwd, ssh_host, ssh_port)
            if t == -1:
                LOG.debug("Liupu DEBUG: ssh connect failed t == -1 " )
            else:
                chan = sshclient.ssh_channel(t)
                if chan == -1:
                    LOG.debug("Liupu DEBUG: ssh channel create failed chan == -1 " )
                else:
                    #pass
                    switch_driver.scan_server_lldp(chan)
                sshclient.ssh_close(chan, t)
            LOG.debug("Liupu DEBUG: ssh connect end ! " )
    ...
    def _parse_remote_switch_conf(self):
        ...
        LOG.info("######### cfg.remote_switch_server: %s", cfg.CONF.SWITCHAGENT.remote_switch_server)
        switch_tmp =  cfg.CONF.SWITCHAGENT.remote_switch_server
        if ';' in switch_tmp:
            switch_server_cfg = switch_tmp.split(';')
            LOG.info("######### switch_server_cfg: %s", switch_server_cfg)
        
        for entry in switch_server_cfg:
            entry = entry.strip()
            LOG.info("######### entry new: %s", entry)
            if ':' in entry:
                try:
                    index, username, password, server, sshport = entry.split(':')
                    LOG.info("######### index: %s , username = %s ,pass =%s ", index,username,password)
                    LOG.info("######### index: %s , server = %s ,sshport =%s ", index,server,sshport)
        ...
        LOG.info("_parse_remote_switch_conf: %s", self.switch_server_cfg)
    ...
    def update_network(self, context, id, network):
        ...
        print 'The network id is %s' % (id)
        segmentation_id =  network['network']['segmentation_id']
        print 'The segmentation id is %s' % (segmentation_id)
        session = context.session
        with session.begin(subtransactions=True):
            binding = rgos_vlanmgr.get_network_binding(session, id)
            if binding.network_type in [constants.TYPE_VLAN,
                                          constants.TYPE_FLAT]:
